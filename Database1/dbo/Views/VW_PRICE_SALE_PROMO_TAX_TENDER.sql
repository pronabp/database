CREATE VIEW [dbo].[VW_PRICE_SALE_PROMO_TAX_TENDER]
...
C.TenderChange	,
C.ChangeCurrency	
FROM [VW_PRICE_SALE_TAX_REDEMPTION] A INNER JOIN DIM_CALENDAR B ON A.BusinessDayDate=B.CLNDR_DATE
LEFT OUTER JOIN (select * from EG_POS_TENDERLINE
where RefusalTypeCode is NULL
and TenderType IN ('Cash','CreditDebit') 
and Amount <> 0) -- Avi's Change
C ON
A.BusinessUnitID=C.BusinessUnitID and 
A.POSTXID=C.POSTXID and
A.BusinessDayDate=C.BusinessDayDate
where C.VoidFlag is NULL
and A.ItemDescription is not NULL;